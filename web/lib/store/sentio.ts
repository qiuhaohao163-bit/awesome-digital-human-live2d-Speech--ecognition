...
// ==================== 唤醒词检测 ==================
interface SentioWakewordState {
    enable: boolean,
    asrEngine: string,
    asrSettings: { [key: string]: any },
    wakewords: string,
    isListening: boolean,
    lastDetectedText: string,
    setEnable: (enable: boolean) => void,
    setAsrEngine: (engine: string) => void,
    setAsrSettings: (settings: { [key: string]: any }) => void,
    setWakewords: (wakewords: string) => void,
    setIsListening: (isListening: boolean) => void,
    setLastDetectedText: (text: string) => void
}

export const useSentioWakewordStore = create<SentioWakewordState>()(
    persist(
        (set) => ({
            enable: false,
            asrEngine: "",
            asrSettings: {},
            wakewords: "你好小助手,小助手,唤醒",
            isListening: false,
            lastDetectedText: "",
            setEnable: (enable: boolean) => set((state) => ({ enable: enable })),
            setAsrEngine: (engine: string) => set((state) => ({ asrEngine: engine })),
            setAsrSettings: (settings: { [key: string]: any }) => set((state) => ({ asrSettings: settings })),
            setWakewords: (wakewords: string) => set((state) => ({ wakewords: wakewords })),
            setIsListening: (isListening: boolean) => set((state) => ({ isListening: isListening })),
            setLastDetectedText: (text: string) => set((state) => ({ lastDetectedText: text }))
        }),
        {
            name: 'sentio-wakeword-storage',
        }
    )
)